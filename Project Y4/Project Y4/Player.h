...
public:
	...
	void Update(sf::Vector2f g, sf::Vector2f collisionForce); //checks if the collisions bounding sphere should be drawn(debug option). acumulates all the forces acting on the player and creates one force from them to be applied to the players shapes.
...
private:
	...
	float acceleration; //The acceleration that is applied to the speed when the player is attempting to move.
